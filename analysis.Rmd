...
processed_data %>% 
  ...
  plot_count_and_prop(doctor_visit, FALSE, x_title = "How often do patients meet with their neurologist?")
```
...
processed_data %>%
  ...
  mutate(n_all = sum(n),
         percentage = n / n_all * 100) %>% 
  ggplot() +
  aes(x = use_physiotherapist, y = percentage, fill = symptom_type) +
  geom_bar(stat = "identity", position = "dodge") +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 100), labels = scales::label_percent(scale = 1)) +
  labs(
    x = "Does the patient go to physiotherapy?",
    y = "Percentage",
    fill = "Symptom type"
  ) +
  theme_classic() +
